<?php 
    ...
    if($limpiar_tablas)
    {
        ...
        // ejecuta_delete($db_postgresql, "TRUNCATE TABLE ordenes CASCADE");
            ...
            echo 'Borrando tasascambios...';
            ejecuta_delete($db_postgresql, "TRUNCATE TABLE tasascambios CASCADE");
        ...
        // /**/ ejecuta_update($db_postgresql, "ALTER SEQUENCE planesdestinos_idplandestino_seq RESTART WITH 1");
            /**/ ejecuta_update($db_postgresql, "ALTER SEQUENCE tasacambio_idtasacambio_seq RESTART WITH 1");
    }

    // TASAS DE CAMBIO *******************************************************************************************************************************************************************************
        echo 'Migrando Tabla: Tasas de Cambio';
        
        $mysql    = $db_mysql->query("SELECT 
                                        currency as idmoneda,
                                        valor as valor,
                                        horabanco as fechabanco,
                                        horainsert as fechainsertado,
                                        tipo as idtipotasa,
                                        'NULL' as idagencia,
        ...
        foreach($registros as $registro)
        {
            ...
            $idmoneda               = ejecuta_select($db_postgresql, "SELECT idmoneda FROM monedas WHERE codigo = '$idmoneda'", "idmoneda");
            $valor                  = $registro['valor'];
            $fechabanco             = ($registro['fechabanco'] == '0000-00-00 00:00:00') ? '2000-01-01 00:00:00' : $registro['fechabanco'];
            $fechainsertado         = ($registro['fechainsertado'] == '0000-00-00 00:00:00') ? '2000-01-01 00:00:00' : $registro['fechainsertado'];
            $idtipotasa             = $registro['idtipotasa'];
            $idagencia              = $registro['idagencia'];
            $idusuario              = $registro['idusuario'];

            $insert = "INSERT INTO tasascambios (
                                        idmoneda,
                                        valor,
                                        fechabanco,
                                        fechainsertado,
    ...
    //SECUENCIA TASAS CAMBIOS
        $secuencia = ejecuta_select($db_postgresql, "SELECT MAX(idtasacambio) as idtasacambio FROM tasascambios", "idtasacambio");
        $secuencia = "ALTER SEQUENCE tasacambio_idtasacambio_seq RESTART WITH ".$secuencia; 
    ...
    //                 orders.fecha >= '2022-01-01'
    //                 AND orders.fecha <= '2022-12-31'
    //                 ORDER BY id ASC 
    //             ";
