<?php
    ...
    // VISTAS ANUALES: VISTAS QUE SON GENERADAS POR AÑOS 
    
        $vistas = ejecuta_select($db_postgresql, "SELECT idvista, nombrevista FROM vistasmaterializadas WHERE idstatus = 1 AND vistaanual = true AND actualizarautomatico = true");

        foreach($vistas['resultado'] as $vista)
        {
            $idvista        = $vista['idvista'];
            $nombrevista    = $vista['nombrevista'].'_'. $ano_hoy;
    ...
    // // VISTAS CON NOMBRES SIN AÑOS

    //     $vistas = ejecuta_select($db_postgresql, "SELECT idvista, nombrevista FROM vistasmaterializadas WHERE idstatus = 1 AND vistaanual = false");
